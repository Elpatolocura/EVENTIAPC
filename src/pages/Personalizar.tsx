  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="font-orbitron font-extrabold uppercase text-lg text-gray-900">{t('personalizar.titulo')}</h1>
      </div>
      ...
      <div className="space-y-6">
        <div className="bg-white border-2 border-black shadow-[2px_2px_0px_#000] rounded-lg overflow-hidden">
          ...
          <div className="p-6">
            <h2 className="font-orbitron font-semibold uppercase text-sm text-gray-900 mb-4">{t('personalizar.color')}</h2>
            ...
            <p className="font-share-tech text-slate-500 text-xs mt-3">
              {t('personalizar.seleccionaste')}: <span className="font-medium capitalize">{colors.find((c) => c.value === color)?.name}</span>
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-black shadow-[2px_2px_0px_#000] rounded-lg overflow-hidden">
          <div className="h-1.5 bg-gradient-to-r from-cyan-500 via-purple-500 to-fuchsia-500" />
          <div className="p-6">
            <h2 className="font-orbitron font-semibold uppercase text-sm text-gray-900 mb-4">{t('personalizar.tema')}</h2>
            <div className="grid grid-cols-2 gap-3">
              {themes.map((t) => (
                <button
                  ...
                  className={`p-4 rounded-lg border-2 text-left transition-all cursor-pointer hover:shadow-[3px_3px_0px_#000] ${
                    theme === t.value
                      ? 'border-accent-secondary bg-accent-light shadow-[3px_3px_0px_var(--accent-secondary)]'
                      : 'border-black bg-gray-50 hover:bg-white shadow-[2px_2px_0px_#000]'
                  }`}
                >
                  <div className={`w-full h-20 rounded-lg mb-3 ${t.preview} border flex items-center justify-center`}>
                    <div className={`${theme === t.value ? 'text-accent-secondary' : 'text-gray-400'}`}>
                      {t.icon}
                    </div>
                  </div>
                  <p className="text-sm font-orbitron font-semibold text-gray-900">{t.label}</p>
                  <p className="text-xs font-share-tech text-gray-500 mt-0.5">{t.desc}</p>
                </button>
              ))}
            </div>
          </div>
